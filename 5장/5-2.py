...
@app.route('/lol')
def lol():
    return render_template('lol_rank.html')

#선수 조회 처리
@app.route('/lol_show', methods=['POST']) 
def lol_show():
    #폼 입력값 가져오기
    ra = request.form['rank']

    #데이터베이스 연결 및 조회
    conn = sqlite3.connect('lol rank.db') 
    conn.row_factory = sqlite3.Row 
    cursor = conn.cursor() 
    cursor.execute(''' select * from rank where rank = ? ''', (ra,)) 
    row = cursor.fetchone()
    conn.close()

    outstr = "<h2>랭커조회</h2><br><hr>" 

    if row != None: 
        outstr += "rank: %s <br>" % row['rank'] 
        outstr += "name: %s <br>" % row['name'] 
        outstr += "tier: %s <br>" % row['tier'] 
        outstr += "point: %s <br>" % row['point']
    else: 
        outstr += "선수데이터가없습니다!!"
...
@app.route('/fifa4')
def fufa4():
    return render_template('fifa4_rank.html')

@app.route('/fifa4_show', methods=['POST']) 
def fifa4_show():
    #폼 입력값 가져오기
    ra = request.form['rank']
...
@app.route('/valorant')
def valo():
    return render_template('valo_rank.html')

@app.route('/valo_show', methods=['POST']) 
def valo_show():
    #폼 입력값 가져오기
    ra = request.form['rank']
